                ...
                <div class="space-y-5 p-6">
                    <div>
                        <label for="rincian" class="mb-1.5 block text-sm font-semibold text-slate-700">
                            Rincian Informasi yang Diminta <span class="text-red-500">*</span>
                        </label>
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div>
                            <label for="cara_memperoleh" class="mb-1.5 block text-sm font-semibold text-slate-700">
                        ...
                        <div>
                            <label for="cara_pengiriman" class="mb-1.5 block text-sm font-semibold text-slate-700">
                                Cara Pengiriman Informasi <span class="text-red-500">*</span>
                            </label>
    ...
    @unless ($isLoggedIn)
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const fileInput = document.getElementById('file_identitas');
                const scanButton = document.getElementById('ktp-ocr-button');
                const buttonText = document.getElementById('ktp-ocr-button-text');
                const buttonIcon = document.getElementById('ktp-ocr-icon');
                const spinner = document.getElementById('ktp-ocr-spinner');
                const statusBox = document.getElementById('ktp-ocr-status');
                const previewWrapper = document.getElementById('ktp-preview-wrapper');
                const preview = document.getElementById('ktp-preview');
                const csrfToken = document.querySelector('input[name="_token"]')?.value;
                const ocrUrl = @json(route('public.permohonan.ocr'));
                ...
                const showStatus = (type, message, warnings = []) => {
                    const classes = {
                ...
                fileInput.addEventListener('change', () => {
                    ...
                    if (file.size > 5 * 1024 * 1024) {
                        scanButton.disabled = true;
                        showStatus('error', 'Ukuran file melebihi 5 MB. Pilih file yang lebih kecil.');
                        return;
                    }
                    ...
                    showStatus('info', 'Foto siap dibaca. Tekan tombol “Baca Data KTP Otomatis”.');
                });
                ...
                scanButton.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        setFieldValue('nomor_identitas', data.nik);
                        setFieldValue('nama_pemohon', data.nama);
                        setFieldValue('tmp_lahir', data.tempat_lahir);
                        setFieldValue('tgl_lahir', data.tanggal_lahir);
                        setFieldValue('l_kelamin', data.jenis_kelamin);
                        setFieldValue('pekerjaan_pemohon', data.pekerjaan);
                        setFieldValue('alamat_pemohon', data.alamat);
                        setFieldValue('desa_kel', data.desa_kel);
                        setFieldValue('kecamatan', data.kecamatan);
                        setFieldValue('kota_kab', data.kota_kab);
                        setFieldValue('provinsi', data.provinsi);

                        const completeness = Number(payload.completeness ?? 0);
                        const message = `${payload.message} Kelengkapan hasil: ${completeness}%.`;
