    @php
        $heroSlides = App\Models\Slider::query()
            ->orderByDesc('id')
            ->get()
            ->map(function ($slide) {
                $image = $slide->banner
                    ? (\Illuminate\Support\Str::startsWith($slide->banner, ['http://', 'https://'])
                        ? $slide->banner
                        : asset('storage/' . ltrim($slide->banner, '/')))
                    : asset('assets/img/banner/alunbatu.jpg');

                return [
                    'image' => $image,
                    'alt' => $slide->title ?: 'PPID Kota Batu',
                ];
            })
            ->values()
            ->all();
    ...
    <section class="relative overflow-hidden pb-24 pt-28 lg:pb-28">
        ...
        {{-- Hero Content --}}
        <div
            class="relative z-20 mx-auto flex min-h-[80vh] max-w-6xl items-center justify-center px-6 text-center lg:px-8">
            <div class="max-w-2xl">
        ...
        <div class="absolute bottom-0 left-0 z-20 w-full overflow-hidden leading-[0]">
            <svg class="relative block h-[80px] w-full" viewBox="0 0 1200 120" preserveAspectRatio="none"
        ...
        {{-- Slide Indicators --}}
        <div
            class="absolute bottom-32 left-1/2 z-30 flex -translate-x-1/2 gap-2 lg:bottom-40 lg:left-16 lg:translate-x-0">
            @foreach ($heroSlides as $index => $slide)
                <span
                    class="hero-dot h-1.5 rounded-full bg-white/40 transition-all duration-500 {{ $index === 0 ? 'w-8 bg-white' : 'w-4' }}"
                    data-dot="{{ $index }}" role="button" tabindex="0"
                    aria-label="Pilih slide {{ $index + 1 }}"></span>
            @endforeach
        </div>
    </section>
